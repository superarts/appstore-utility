...
for ($i = 1; $i < count($a); $i++)
{
	...
	$name = get_item($a[$i], 6);
	$type = get_item($a[$i], 7);
	$count = get_item($a[$i], 8);
	$commission = get_item($a[$i], 9);
	$begin = get_item($a[$i], 10);
	$end = get_item($a[$i], 11);
	$currency = get_item($a[$i], 12);
	$country = get_item($a[$i], 13);
	$id = get_item($a[$i], 19);
	$price = get_item($a[$i], 20);
	...
	//echo "$i:\t$sku\t$name\n";
	echo "$type\t$count\t$commission\t$begin\t$end\t$currency\t$country\t$id\t$price\n";
}
